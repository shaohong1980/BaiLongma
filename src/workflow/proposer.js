...
const NODE_CATALOG = [
  ...
  { type: 'merge', note: '汇聚节点。自动收集所有上游节点输出合并成 items，无配置', example: '{ "id":"mg1","type":"merge","name":"汇聚","config":{},"next":"..." }' },
  { type: 'transform', note: '转换节点。config.set = { key: "=expr" }，对上游输出求值并合并（expr 可访问 item / context）', example: '{ "id":"tf1","type":"transform","name":"加字段","config":{"set":{"full_name":"=item.first + \\" \\" + item.last"}},"next":"..." }' },
  { type: 'sub_workflow', note: '子流程复用。config.workflow_id（已保存工作流）或 config.workflow（内联子图）二选一；config.inputs 传子流程输入', example: '{ "id":"sw1","type":"sub_workflow","name":"跑子流程","config":{"workflow_id":"flow-123","inputs":{"repo":"=item.repo"}},"next":"..." }' },

...
import { runStartupScans, warmupLocalEmbeddingAsync, reportStartupProgress } from "./runtime/startup.js"
...
import { getSystemInfoBlock, getBatteryBlock } from './system-info.js'
import { getDesktopBlock } from './desktop-scanner.js'
import { getInstalledSoftwareBlock } from './installed-software-scanner.js'
import { getGeoWeatherBlock } from './geo-weather.js'
import { buildDelegationDiscoveryContext } from './agents/registry.js'
...
const { geoResult, startupSkills } = await runStartupScans()
warmupLocalEmbeddingAsync()
...
configureRecognizerScheduler({
  onResult: async (memories) => {
    emitEvent('memories_written', { count: memories?.length || 0, memories: memories || [] })
    const evolved = await recordSelfEvolutionFromMemories(memories || [], { emitEvent })
...
// Wave 2：查 conversations 表，主用户是否有微信渠道历史（触发 WeChat Outbound 段注入）。
function hasWechatHistoryOnRecord() {
  try {
    const row = getDB().prepare(`
      SELECT id FROM conversations
      WHERE (from_id = ? OR to_id = ?)
        AND (channel LIKE 'WECHAT%' OR from_id LIKE 'wechat:%' OR to_id LIKE 'wechat:%')
      LIMIT 1
    `).get(PRIMARY_USER_ID, PRIMARY_USER_ID)
    return !!row
  } catch {
    return false
  }
...
async function runTurn(input, label, msg = null) {
  ...
  try {
    ...
    const agentName = getConfig('agent_name') || '爻台'
    ...
    // hasWechatHistory：查 conversations 表看主用户是否在微信渠道出现过（触发 WeChat Outbound 段）。
    // hasActiveFocus：有前台线索（正在聚焦的注意力）即视为 focus banner active。
    let systemPrompt = buildSystemPrompt({
      ...
      hasWechatHistory: hasWechatHistoryOnRecord(),
      hasActiveFocus: !!getForegroundThread(state),

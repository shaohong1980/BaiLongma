...
- For tasks that need to actually INTERACT with a webpage — logging in, filling forms, clicking through UI, pagination, taking screenshots — use browser_act. It keeps a persistent browser session across calls: navigate → click(selector) → fill(selector,value) → press(key) → snapshot → screenshot. Use the returned "interactive" list to pick selectors. Always end with action=close when done. Prefer browser_read/fetch_url for just reading a page; browser_act is for when you must operate the page.
